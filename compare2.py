...
Con_Matr = []
# 数据导入
data = np.load('D:\RF_CNN\data/data_g4g67.npy')
label = np.load('D:\RF_CNN\data/label_g4g67.npy')
...
def models(input_shape):
    ...
    # 初始层
    conv1 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(input_shape)
    conv1 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv1)
    conv1 = Activation('relu')(conv1)

    # Residual Block1
    conv2 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv1)
    conv2 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv2)
    conv2 = Activation('relu')(conv2)
    conv2 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv2)
    conv2 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv2)
    conv2 = Add()([conv1, conv2])
    conv2 = Activation('relu')(conv2)
    conv2 = MaxPooling1D(pool_size=5, strides=2)(conv2)

    # Residual Block2
    conv3 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv2)
    conv3 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv3)
    conv3 = Activation('relu')(conv3)
    conv3 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv3)
    conv3 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv3)
    conv3 = Add()([conv2, conv3])
    conv3 = Activation('relu')(conv3)
    conv3 = MaxPooling1D(pool_size=5, strides=2)(conv3)

    # Residual Block3
    conv4 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv3)
    conv4 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv4)
    conv4 = Activation('relu')(conv4)
    conv4 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv4)
    conv4 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv4)
    conv4 = Add()([conv3, conv4])
    conv4 = Activation('relu')(conv4)
    conv4 = MaxPooling1D(pool_size=5, strides=2)(conv4)

    # Residual Block4
    conv5 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv4)
    conv5 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv5)
    conv5 = Activation('relu')(conv5)
    conv5 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv5)
    conv5 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv5)
    conv5 = Add()([conv4, conv5])
    conv5 = Activation('relu')(conv5)
    conv5 = MaxPooling1D(pool_size=5, strides=2)(conv5)

    # Residual Block5
    conv6 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv5)
    conv6 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv6)
    conv6 = Activation('relu')(conv6)
    conv6 = Conv1D(filters=32, kernel_size=5, strides=1, padding='same')(conv6)
    conv6 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv6)
    conv6 = Add()([conv5, conv6])
    conv6 = Activation('relu')(conv6)
    conv6 = MaxPooling1D(pool_size=5, strides=2)(conv6)
    out = Flatten()(conv6)
    out = Dense(32, activation='relu')(out)
    out = Dense(2, activation='softmax')(out)
    out = Model(inputs=[input_shape], outputs=[out], name="compare2")
    return out
...
filepath = "D:/RF_CNN/compare/model2BN_g467_x.hdf5"  # 保存模型的路径
