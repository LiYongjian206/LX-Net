...
def models(input_shape):
    ...
    # 初始层
    conv1 = Conv1D(filters=8, kernel_size=7, strides=2, padding='same')(input_shape)
    conv1 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv1)
    conv1 = Activation('relu')(conv1)

    # Inverted Residual Block1
    conv2 = Conv1D(filters=48, kernel_size=1, strides=1, padding='same')(conv1)
    conv2 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv2)
    conv2 = Activation('relu')(conv2)
    conv2 = DepthwiseConv1D(kernel_size=7, strides=2, padding='same')(conv2)
    conv2 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv2)
    conv2 = Activation('relu')(conv2)
    conv2 = Conv1D(filters=8, kernel_size=1, strides=1, padding='same')(conv2)
    conv2 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv2)
    conv2 = Activation('relu')(conv2)

    # Inverted Residual Block2
    conv3 = Conv1D(filters=48, kernel_size=1, strides=1, padding='same')(conv2)
    conv3 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv3)
    conv3 = Activation('relu')(conv3)
    conv3 = DepthwiseConv1D(kernel_size=7, strides=2, padding='same')(conv3)
    conv3 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv3)
    conv3 = Activation('relu')(conv3)
    conv3 = Conv1D(filters=12, kernel_size=1, strides=1, padding='same')(conv3)
    conv3 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv3)
    conv3 = Activation('relu')(conv3)

    # Inverted Residual Block3
    conv4 = Conv1D(filters=72, kernel_size=1, strides=1, padding='same')(conv3)
    conv4 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv4)
    conv4 = Activation('relu')(conv4)
    conv4 = DepthwiseConv1D(kernel_size=5, strides=2, padding='same')(conv4)
    conv4 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv4)
    conv4 = Activation('relu')(conv4)
    conv4 = Conv1D(filters=16, kernel_size=1, strides=1, padding='same')(conv4)
    conv4 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv4)
    conv4 = Activation('relu')(conv4)

    # Inverted Residual Block4
    conv5 = Conv1D(filters=96, kernel_size=1, strides=1, padding='same')(conv4)
    conv5 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv5)
    conv5 = Activation('relu')(conv5)
    conv5 = DepthwiseConv1D(kernel_size=5, strides=2, padding='same')(conv5)
    conv5 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv5)
    conv5 = Activation('relu')(conv5)
    conv5 = Conv1D(filters=24, kernel_size=1, strides=1, padding='same')(conv5)
    conv5 = BatchNormalization(momentum=0.99, epsilon=0.001)(conv5)
    conv5 = Activation('relu')(conv5)

    out = GlobalAveragePooling1D()(conv5)
    out = Dense(2, activation='softmax')(out)

    out = Model(inputs=[input_shape], outputs=[out], name="compare1")
    return out
...
filepath = "D:/RF_CNN/compare/model1_g467_x.hdf5"  # 保存模型的路径
